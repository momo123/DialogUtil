package com.wx.project.dialogutil.widget;
...
public class DateWheelView extends View {
    ...
    public class  WheelType{
        ...
        public static final int DAY = 1002;
        public static final int HOUR = 1003;
        public static final int MINUTE = 1004;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (controlWidth != 0) {
            setMeasuredDimension(getMeasuredWidth(), (int)(itemNumber * unitHeight));
            controlWidth = getMeasuredWidth();
        }
        if (controlWidth  <= 0){
            if (mContext != null) {
                controlWidth = (int) (display.getWidth() * 0.6);
            }
        }
...
//                    	LogUtils.d("sleep" + e.toString());
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        TypedArray attribute = context.obtainStyledAttributes(attrs,
                R.styleable.BaseWheelView);
        unitHeight = (int) attribute.getDimension(
                R.styleable.BaseWheelView_BaseunitHight, 32);
        normalFont = attribute.getDimension(
                R.styleable.BaseWheelView_BasenormalTextSize, 14.0f);
        selectedFont = attribute.getDimension(
                R.styleable.BaseWheelView_BaseselectedTextSize, 20.0f);
        itemNumber = attribute.getInt(R.styleable.BaseWheelView_BaseitemNumber, 7);
        normalColor = attribute.getColor(
                R.styleable.BaseWheelView_BasenormalTextColor, 0xff000000);
        selectedColor = attribute.getColor(
                R.styleable.BaseWheelView_BaseselectedTextColor, 0xffff0000);
        lineColor = attribute.getColor(R.styleable.BaseWheelView_BaselineColor,
                0xff000000);
        lineWidth = attribute.getDimension(R.styleable.BaseWheelView_BaselineHeight, 2f);
        maskHight = attribute.getDimension(R.styleable.BaseWheelView_BasemaskHight,
                48.0f);
        noEmpty = attribute.getBoolean(R.styleable.BaseWheelView_BasenoEmpty, true);
        isEnable = attribute
                .getBoolean(R.styleable.BaseWheelView_BaseisEnable, true);
        attribute.recycle();
     ...
     * @param data
     */
    public void setData(ArrayList<String> data,int type) {
        wheelType = type;
        this.dataList = data;
        initData();
    }
    ...
    private class ItemObject {
        ...
        public void drawSelf(Canvas canvas) {
            ...
            String typeTip = "";
            if (controlWidth > display.getWidth()/5)
                switch (wheelType){
                    case WheelType.YEAR:
                        typeTip = " 年";
                        break;
                    case WheelType.MONTH:
                        typeTip = " 月";
                        break;
                    case WheelType.DAY:
                        typeTip = " 日";
            ...
            // 绘制内容
            canvas.drawText(itemText+typeTip, x + controlWidth / 2 - (float)(textRect.width())
                            / 2, y + move + unitHeight / 2 + (float)(textRect.height()) / 2,
                    textPaint);
        ...
        public boolean isInView() {
            if (y + move > controlHeight
                    || (y + move + unitHeight / 2 + (float)(textRect.height()) / 2) < 0)
                return false;
            return true;
        }
